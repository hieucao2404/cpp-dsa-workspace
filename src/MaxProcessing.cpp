...
Maze::Maze() : exitMarker('e'), entryMarker('m'), visited('.'), passage('0'), wall('l')
{
    Stack<char *> mazeRows;
    char str[80], *s;
    int col, row = 0;
    ...
    while (cin >> str)
    {
        row++;
        cols = strlen(str);
        s = new char[cols+3]; // 2 more cells for borderline
        mazeRows.push(s);
        strcpy(s+1, str);
        s[0] = s[cols+1] = wall; // fill the border line cells with walls
        s[cols + 2] = '\0';
    ...
    for (row = rows; !mazeRows.empty(); row--){
